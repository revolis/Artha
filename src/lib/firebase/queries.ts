...
export async function getDashboardYearData(
  db: ReturnType<typeof createFirestoreDbClient>,
  userId: string,
  year: number
...
export async function getAvailableYears(db: ReturnType<typeof createFirestoreDbClient>, userId: string) {
  ...
  (entries ?? []).forEach((entry: { entry_date: string }) => {
  ...
  if (missingYears.length > 0) {
    await db.from("financial_years").upsert(
